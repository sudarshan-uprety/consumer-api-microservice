        ...
        stage('Sync Deployments with rsync') {
            steps {
                script {
                    ...
                    def composeUpCommand = "sudo docker-compose up --build -d auth_db_${env.BRANCH_NAME} auth_service_${env.BRANCH_NAME}"
                    ...
                    withCredentials([ 
                        ...
                        sh """
                            rsync -avz --delete --exclude='.git/' --exclude='.github/' -e "ssh -i \$SSH_KEY_PATH -p \$SSH_PORT -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null" ./ .env \${SERVER_USER}@\${SERVER_HOST}:/home/\${SERVER_USER}/sudarshan/microservices/authentication/${env.BRANCH_NAME}/
                        """

                        // Step 3: SSH to start the docker containers
                        sh """
                            ssh -i \$SSH_KEY_PATH -p \$SSH_PORT -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \${SERVER_USER}@\${SERVER_HOST} "cd /home/\${SERVER_USER}/sudarshan/microservices/authentication/${env.BRANCH_NAME} && ${composeUpCommand}"
                        """
